...
export function renderDetails(exercise) {
  if (!dom.detailsTitle || !dom.detailsDesc || !dom.detailsParams) return;
...
export function updateFullscreenButton(isFullscreen) {
  ...
  dom.fsBtn.textContent =
    isFullscreen ? "Wyłącz pełny ekran" : "Pełny ekran";
...
export function updateWakeLockButton(isActive) {
  ...
  dom.wakeBtn.textContent =
    isActive ? "Blokada ekranu (Tak)" : "Blokada ekranu (Nie)";
...
export function updateSoundButton(isActive) {
  ...
  dom.soundBtn.textContent =
    isActive ? "Dźwięki włączone" : "Dźwięki wyłączone";
